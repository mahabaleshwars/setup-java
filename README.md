...
**NOTE:** On Ubuntu runners, commands executed via `sudo` do not inherit the `JAVA_HOME` and `PATH` set by `setup-java` and will fall back to the runner image's system-default JDK. For additional context, refer to the [related discussion](https://github.com/actions/setup-java/issues/997).
