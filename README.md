...
**NOTE:** Oracle JDK 17 releases up to and including 17.0.12 are available under the Oracle No-Fee Terms and Conditions (NFTC) license, which permits free commercial and production use. Starting from 17.0.13, Oracle JDK 17 is only available under the Oracle Technology Network (OTN) license, which is more restrictive and typically requires a paid subscription. When using `java-version: '17'` with the `oracle` distribution, it is recommended to pin to version `17.0.12` or earlier. Alternatively, consider upgrading to Oracle JDK 21 or later, which remains available under the NFTC license.
